...
| **스테이블코인** | x402가 올라타 있는 레일의 크기. 30일 헤드라인 + 소매형 거래액·건수의 **일간 추이 차트**(30/90/180일) + 체인·자산 분포 |
...
- **마지막 날은 적재가 덜 끝나 있다.** 검증 때 최신일 거래액이 직전 7일 중앙값의 10%
  수준이었다. 그대로 그리면 차트에 없는 급락이 생기고 30일 합계도 하루치 모자라게 나온다.
  수집기가 직전 7일 중앙값의 절반에 못 미치는 마지막 날을 잘라내고, 버린 날짜를
  `partialDayDropped`에 남긴다. 이걸 적용하자 30일 소매형이 $7.91B / 1.749억건으로
  Visa 페이지($7.9B / 175.6M)와 사실상 일치했다.
- **Visa의 "Adjusted"와 이 테이블의 합계는 다르다.** 페이지 상단 `Adjusted Transaction Volume`
  ...
  훨씬 크다. 대시보드가 페이지와 맞춰 쓰는 값은 **`Retail Sized`** 쪽이고, 나머지는
  "전체"로만 표기한다.
- **차트는 축이 하나다.** 거래액과 건수는 자릿수가 달라 한 판에 겹치지 않고 각각 자기 축을
  가진 차트로 나눈다. 축 두 개짜리 한 판은 두 스케일을 임의로 맞추느라 없는 상관을 만든다.
